...
Tilemap::Tilemap(SDL_Texture *_Tilesheet)
    : Tilesheet(_Tilesheet), X_OFFSET(0), Y_OFFSET(0), MAP_WIDTH(0),
      MAP_HEIGHT(0) {
...
void Tilemap::Draw() {
  ...
  for (uint16_t i = 0; i < MAP_HEIGHT; ++i) {
    for (uint16_t j = 0; j < MAP_WIDTH; ++j) {
      ...
      X_COORD += TILE_WIDTH;
      JamEngine::Instance()->drawTexture(Tilesheet, &TilesheetSrc,
                                         &RendererDest, 0);
    }
    Y_COORD += TILE_HEIGHT;
    X_COORD = X_OFFSET;
...
TILE::TILE(uint16_t _x, uint16_t _y) : X(_x), Y(_y) {}